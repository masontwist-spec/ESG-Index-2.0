...
function renderTable() {
  ...
  document.getElementById('tableBody').innerHTML = rows.map(d => {
    ...
    return `<tr>
      ...
      <td><div class="metric-cell"><div class="tiny-track"><div class="tiny-fill bad" style="width:${d.Climate_Targets * 100}%"></div></div><div class="pct">${pct(d.Climate_Targets)}</div></div></td>
      <td><div class="metric-cell"><div class="tiny-track"><div class="tiny-fill bad" style="width:${d.Investment_Transition * 100}%"></div></div><div class="pct">${pct(d.Investment_Transition)}</div></div></td>
      <td><div class="metric-cell"><div class="tiny-track"><div class="tiny-fill bad" style="width:${d.Climate_Reporting * 100}%"></div></div><div class="pct">${pct(d.Climate_Reporting)}</div></div></td>
      <td><div class="metric-cell"><div class="tiny-track"><div class="tiny-fill ${tier === 'poor' || tier === 'wasteful' ? 'bad' : ''}" style="width:${d.Environment_Score * 100}%"></div></div><div class="score-value" style="color:${color}">${pct(d.Environment_Score)}</div></div></td>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('th[data-sort]').forEach(th => th.addEventListener('click', () => {
    ...
    else { sortCol = col; sortAsc = col === 'Environment_Score' || col === 'rank'; }
    renderTable();
  }));
